...
beats fetching**: bundled images render with the network off, and ten remote fetches on
conference wifi mid-demo is exactly the failure you spent the morning designing out.

Real-dish URLs, if you use them — they are also listed in **Correction A**:
...
Label photos (`*-label.jpg`) are your provenance for the allergen data — worth having on hand if
a judge asks where the allergens came from. Don't ship them in the app.
